...
class TestIeArd(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.observation_data = pl.DataFrame(
            {
                "USUBJID": ["02", "03", "03"],
                "PARAM": ["Criterion 1 not met", "Criterion 1 not met", "Criterion 2 not met"],
                "PARAMCAT": ["Inclusion", "Inclusion", "Exclusion"],
                "AFLAG": ["Y", "Y", "Y"],
            }
        )
    ...
    def test_ie_summary_ard_basic(self) -> None:
        ...
        enrolled_trt_a = ard.filter(
            (pl.col("__index__") == "Total Subjects Screened")
            & (pl.col("__group__") == "Treatment A")
        )["__value__"].item()
        self.assertEqual(enrolled_trt_a, "2")

        # Check Header: "Inclusion Criteria Not Met"
        incl_header_trt_a = ard.filter(
            (pl.col("__index__") == "Inclusion Criteria Not Met")
            & (pl.col("__group__") == "Treatment A")
        )["__value__"].item()
        ...
        c1_trt_a = ard.filter(
            (pl.col("__index__") == "    Criterion 1 not met")
            & (pl.col("__group__") == "Treatment A")
        )["__value__"].item()
...
class TestIePipeline(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.obs = pl.DataFrame(
            {"USUBJID": ["01"], "PARAM": ["C1"], "PARAMCAT": ["Inclusion"], "AFLAG": ["Y"]}
        )
