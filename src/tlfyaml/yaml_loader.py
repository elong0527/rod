...
class YamlInheritanceLoader:
    ...
    def deep_merge_lists(self, list1: list[dict[str, Any]], list2: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        is_keyword_list = all(isinstance(i, dict) and "name" in i for i in list2) and all(
            isinstance(i, dict) and "name" in i for i in list1
        )

        if is_keyword_list:
            merged_by_name = {item["name"]: item for item in list1}
            for item in list2:
    ...
    def _deep_merge(self, dict1: Dict[str, Any], dict2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in dict2.items():
            if key in merged and isinstance(merged[key], list) and isinstance(value, list):
                merged[key] = self.deep_merge_lists(merged[key], value)
